...
function UserProducts() {
  ...
  const fetchProducts = () => {
    ...
    }/product/get-all-products-sorted?sortBy=${sortChoice}&order=${sortOrder}`;
    ...
    <div>
      ...
      <div className="w-full h-25 bg-[#DDA15E] flex mb-12 justify-between items-center">
        <div className="flex justify-start items-center">
          ...
          <p className="text-4xl font-serif my-1 text-[#FEFAE0]">
            | Product Listings{" "}
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-4 justify-center items-center">
        {products.map((product) => (
          <ProductCard key={product._id} data={product} addToCart={addToCart} />
        ))}
      </div>
    </div>
  );
